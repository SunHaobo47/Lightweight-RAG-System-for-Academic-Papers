...
# 1. 增加模型缓存装饰器
@lru_cache(maxsize=1)
def get_embedding_instance(model_name):
    device = "cuda" if torch.cuda.is_available() else "cpu"
    print(f"--- 正在真正加载 Embedding 模型 {model_name} (仅此一次) ---")
...
def load_faiss_index(faiss_path: str, embed_model: str):
    # 这里不再直接 new 模型，而是从缓存拿
    emb = get_embedding_instance(embed_model)
...
def run_batch_evaluation(retriever, rag_chain, question_file: str, output_file: str, max_questions=5, workers=4):
    ...
    def process_row(row):
        ...
        # --- A. 获取原始检索结果 ---
        # retriever 会根据你之前设置的 TOP_K 返回一个 List[Document]
        retrieved_docs = retriever.invoke(question)
        ...
        for i, doc in enumerate(retrieved_docs):
            ...
            src = doc.metadata.get('source', 'Unknown')
            # 格式化每一个分块的内容
            chunk_text = f"[Chunk {i+1} | Source: {src}]\n{doc.page_content}"
            contexts.append(chunk_text)

        # 使用分隔符将所有分块合并为一个巨大的字符串，这就是送给 LLM 的背景知识
        full_context = "\n\n------------------------\n\n".join(contexts)

        # --- C. 送入模型 ---
        # 此时 rag_chain 内部会自动将这个 full_context 填入 Prompt 的 {context} 槽位中
        answer = rag_chain.invoke(question)
        ...
        return {
            ...
            "Retrieved_Context": full_context  # 存入结果，方便后续在 Excel 里审计
        }
    with ThreadPoolExecutor(max_workers=workers) as executor:
        results = list(tqdm(executor.map(process_row, [r for _, r in df_questions.iterrows()]),total=len(df_questions)))
